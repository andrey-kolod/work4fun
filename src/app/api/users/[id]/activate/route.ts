...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // ✅ ИСПРАВЛЕНО: changedBy → changedById
    ...
    await prisma.userStatusHistory.create({
      data: {
        ...
        newStatus: isActive,
        changedById: parseInt(session.user.id), // ИСПРАВЛЕНО ЗДЕСЬ
        reason: reason || '',
    ...
    if (isActive) {
      await audit.activateUser(parseInt(session.user.id), user.id, reason, request);
    } else {
      await audit.deactivateUser(parseInt(session.user.id), user.id, reason, request);
    }
